...
class UsuarioRepositorio:
    ...
    async def buscar_usuarios_por_email_senha(self, email: str, senha: str) -> Usuario | None:
        with self.bd.conectar() as conexao:
            cursor = conexao.cursor()
            cursor.execute('SELECT id, nome, email, senha FROM usuarios WHERE email = ? AND senha = ?', (email, senha))
